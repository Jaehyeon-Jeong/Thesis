...
| `stage5_llm_news_embedding` | LLM-derived news embedding/sentiment context | In progress: OpenAI embedding, FinBERT-only, and FinBERT+F&G bounded FiLM are complete; remaining work is correction/regression analysis, optional prompt/event labels, and final Stage 5 reporting | [README](stage5_llm_news_embedding/README.md), [checklist](stage5_llm_news_embedding/checklist.md), [5-9E result](stage5_llm_news_embedding/checklist_results/5-9E_finbert_fg_bounded_film_results.md), [comparison](stage5_llm_news_embedding/checklist_results/5-9F_stage5_finbert_comparison.md) |
...
- **Stage 5:** the LLM/news representation track is now active. OpenAI headline embeddings and FinBERT headline sentiment have been converted into numeric context for the same frozen Stage2 + bounded FiLM protocol. Embedding-only and FinBERT-only do not beat the visual baseline on accuracy. FinBERT + F&G is a small positive/near-tie result: mean accuracy `0.580569`, ROC-AUC `0.585843`, `+0.001249` accuracy over Stage2 `ohlc_ma_vb`, and `+0.000278` over F&G-only. The margin is small, so the next useful step is correction/regression analysis or optional prompt/event labels rather than another broad embedding grid.
...
| `stage5_llm_news_embedding` | LLM 기반 news embedding/sentiment context | 진행 중: OpenAI embedding, FinBERT-only, FinBERT+F&G bounded FiLM 완료; 남은 작업은 correction/regression 분석, 선택적 prompt/event label, 최종 Stage 5 보고 | [README](stage5_llm_news_embedding/README.md), [checklist](stage5_llm_news_embedding/checklist.md), [5-9E result](stage5_llm_news_embedding/checklist_results/5-9E_finbert_fg_bounded_film_results.md), [comparison](stage5_llm_news_embedding/checklist_results/5-9F_stage5_finbert_comparison.md) |
