...
| `stage4_film_conditioning` | Compare market-context concat, gating, gamma-only FiLM, and full FiLM on the fixed BTC CNN | v1 four-ablation five-seed run complete; `film_full` is best Stage 4 v1 method but below Stage 2 baseline on average; v2 priorities 1-2 reproduced Stage 2 controls, priority 3 OHLC all-context FiLM runner ready |
...
| `stage4_film_conditioning` | 고정 BTC CNN 위에서 market-context concat, gating, gamma-only FiLM, full FiLM 비교 | v1 four-ablation five-seed run 완료; `film_full`이 Stage 4 v1 best지만 평균적으로 Stage 2 baseline보다 낮음; v2 우선순위 1-2는 Stage 2 control 재현 완료, 우선순위 3 OHLC all-context FiLM runner 준비 완료 |
