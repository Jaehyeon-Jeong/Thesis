...
| `stage4_film_conditioning` | Market-context concat/gating/FiLM | In progress; structured F&G-only FiLM did not robustly beat the Stage 2 visual baseline, so the next track is strict no-leakage news context | [README](stage4_film_conditioning/README.md), [checklist](stage4_film_conditioning/checklist.md) |
...
- **Stage 4:** market-context conditioning is under active refinement. V9 showed that F&G-only numeric FiLM is not robust enough, so the next track is headline-only news context with strict `t-1` alignment.
...
| `stage4_film_conditioning` | Market-context concat/gating/FiLM | 진행 중; structured F&G-only FiLM은 Stage 2 visual baseline을 robust하게 넘지 못했고, 다음 track은 strict no-leakage news context | [README](stage4_film_conditioning/README.md), [checklist](stage4_film_conditioning/checklist.md) |
...
- **Stage 4:** market context를 FiLM/gating/concat으로 붙이는 실험을 진행 중입니다. V9에서 F&G-only numeric FiLM이 robust하지 않다는 것을 확인했고, 다음은 headline-only news context와 strict `t-1` alignment입니다.
