...
| `stage5_llm_news_embedding` | LLM-derived news embedding/sentiment context | In progress: OpenAI embedding and FinBERT sentiment paths are implemented; FinBERT-only is complete, FinBERT+F&G is prepared/running, and final Stage 5 comparison/report remains | [README](stage5_llm_news_embedding/README.md), [checklist](stage5_llm_news_embedding/checklist.md), [5-9E runner](stage5_llm_news_embedding/notebooks/kaggle_stage5_9e_finbert_fg_film_ablation_one_cell.md) |
...
| `stage5_llm_news_embedding` | LLM 기반 news embedding/sentiment context | 진행 중: OpenAI embedding과 FinBERT sentiment 경로 구현 완료; FinBERT-only 결과까지 완료, FinBERT+F&G 실행 준비 완료/진행 중, 최종 비교와 보고만 남음 | [README](stage5_llm_news_embedding/README.md), [checklist](stage5_llm_news_embedding/checklist.md), [5-9E runner](stage5_llm_news_embedding/notebooks/kaggle_stage5_9e_finbert_fg_film_ablation_one_cell.md) |
...
- **Stage 5:** LLM/news representation track은 현재 진행 중입니다. OpenAI headline embedding과 FinBERT headline sentiment를 같은 frozen Stage2 + bounded FiLM protocol의 numeric context로 변환했습니다. Embedding-only와 FinBERT-only는 accuracy 기준으로 visual baseline을 넘지는 못했지만, FinBERT-only는 ranking/calibration을 아주 작게 개선했습니다. 현재는 FinBERT + F&G 결합 실험 후 최종 비교/보고로 마무리하는 흐름입니다.
