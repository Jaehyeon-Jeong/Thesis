...
Kaggle execution standard:
- Use `notebooks/kaggle_stage1_single_horizon_one_cell.md` as the copy-paste
  Kaggle interface.
- The notebook cell calls the scripts above rather than duplicating their
  implementation.
...
Kaggle 실행 표준:
- `notebooks/kaggle_stage1_single_horizon_one_cell.md`를 Kaggle 복붙용 실행
  interface로 사용합니다.
- Notebook cell은 구현을 중복하지 않고 위 script들을 호출합니다.
