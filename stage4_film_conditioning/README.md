...
| `4-N13-5/5A/5B/6` | Macro comparison, cross-context feature audit, selected-combo FiLM, interpretability export | N13-5A completed on 2,399 aligned rows and 126 features; news SVD has the strongest train-only signal, F&G remains the cleanest compact source, and Stage 2 error correlations are weak, so N13-5B should use a small selected-combo vector | [N13-5 result](checklist_results/4-N13-5_macro_context_source_comparison.md), [N13-5A audit](checklist_results/4-N13-5A_cross_context_feature_audit.md), [selected candidates](reports/tables/stage4_n13_5a_cross_context_selected_feature_candidates.csv) |
