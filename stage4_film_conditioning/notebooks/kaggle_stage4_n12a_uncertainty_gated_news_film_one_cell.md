# Kaggle Stage 4 N12-A Uncertainty-Gated News FiLM One Cell
...
This runner executes N12-A, an uncertainty-gated version of the N9
...
news SVD context -> MLP -> uncertainty-gated bounded last-block FiLM
CNN frozen
classifier frozen
uncertainty gate: 4 * p_up_stage2 * (1 - p_up_stage2)
...
- N10-B showed only a small net correction, so N12-A tests whether context
  should intervene mostly when the frozen Stage 2 visual classifier is
  uncertain.
- Confident Stage 2 samples receive a smaller FiLM gate; ambiguous samples
  receive a larger gate.
...
CONTEXT_METHOD = "film_full_uncertainty_gated_last_block"

N12A_GRID_LABEL = "n12a_uncertainty_gated_news_film"
...
# Smoke check only. For the real N12-A run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = f"stage4_{N12A_GRID_LABEL}"
BUNDLE_PATH = Path(f"/kaggle/working/stage4_{N12A_GRID_LABEL}_result_bundle.zip")
...
def assert_required_code():
    ...
    marker_checks = [
        ...
        ("src/stage4_film/runners/context_experiment.py", "film_full_uncertainty_gated_last_block"),
        ("src/stage4_film/runners/context_experiment.py", "configure_stage2_pretrained_context_model"),
        ("src/stage4_film/models/film_stock_cnn.py", "UncertaintyGatedLastBlockFilmContextStockCNN"),
        ("src/stage4_film/models/film_stock_cnn.py", "4.0 * stage2_prob_up * (1.0 - stage2_prob_up)"),
...
def patch_prebuilt_news_config(
    ...
    cfg["stage4_model"].setdefault("film_full_uncertainty_gated_last_block", {})
    cfg["stage4_model"]["film_full_uncertainty_gated_last_block"]["modulation_scale"] = float(scale)
...
def write_result_bundle(seed_csv: Path, mean_std_csv: Path, run_summary_json: Path):
    ...
    include_paths.extend(sorted(tables.glob("stage4_n12a*")))
    include_paths.extend(sorted(tables.glob(f"stage4_{N12A_GRID_LABEL}*")))
...
display(Markdown("# Stage 4 N12-A Uncertainty-Gated News FiLM Seed-Level Results"))
display(seed_df.sort_values(["svd_dim", "modulation_scale", "run_seed"]))

display(Markdown("# Stage 4 N12-A Uncertainty-Gated News FiLM Mean/Std Summary"))
...
display(pd.DataFrame([
    {
        ...
        "roc_auc_mean": 0.5863,
        "note": "Closest news-only bounded FiLM comparison before uncertainty gating.",
    },
    {
    ...
    },
    {
        ...
        "roc_auc_mean": 0.5849,
        "note": "Target baseline before N12-A.",
    },
]))
