# Kaggle Stage 4 N12-B Confidence-Gated News FiLM One Cell
...
This runner executes N12-B, a confidence-gated version of the N9
news-conditioned pretrained/baseline-preserving FiLM setup:

```text
I60 / R20 / ohlc_ma_vb
headline windows: 7d + 20d + 60d
Stage 2 checkpoint -> load learned visual/classifier weights
news SVD context -> MLP -> confidence-gated bounded last-block FiLM
CNN frozen
classifier frozen
confidence gate: abs(2 * p_up_stage2 - 1)
FiLM grid: SVD32/0.02, SVD32/0.05
seeds: 42, 43, 44, 45, 46
```

Purpose:
- N7 trained a Stage2-style CNN from scratch and did not beat the selected
  Stage 2 visual baseline.
- N8-B showed the cleaner baseline-preserving setup: load Stage 2 weights,
  freeze the visual path, and train only bounded context-FiLM correction.
- N9/N10 used the same bounded FiLM strength for all samples.
- N12-B was effectively tied with Stage 2 and only allowed stronger modulation
  near uncertain chart decisions.
- N12-B tests the opposite interpretation: news context may work better as
  reinforcement when the frozen Stage 2 visual classifier is confident.
- Ambiguous Stage 2 samples receive a smaller FiLM gate; confident samples
  receive a larger gate. This may amplify confidently wrong visual decisions,
  so correction/regression and predicted-Up rate must be reviewed.
...
# If auto-detect fails, set this to a folder or zip containing
...
SKIP_GRID_POINTS = set()
CONTEXT_METHOD = "film_full_confidence_gated_last_block"

N12B_GRID_LABEL = "n12b_confidence_gated_news_film"
...
# Smoke check only. For the real N12-B run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = f"stage4_{N12B_GRID_LABEL}"
BUNDLE_PATH = Path(f"/kaggle/working/stage4_{N12B_GRID_LABEL}_result_bundle.zip")
...
def scale_label(scale: float) -> str:
    return f"s{float(scale):.2f}".replace(".", "p")


def should_skip_grid_point(svd_dim: int, scale: float) -> bool:
    rounded = round(float(scale), 4)
    return (int(svd_dim), rounded) in {
        (int(dim), round(float(value), 4)) for dim, value in SKIP_GRID_POINTS
    }


def experiment_suffix(svd_dim: int, scale: float) -> str:
    return (
        f"news_tfidf_svd{int(svd_dim)}_w7_20_60_"
        f"pretrained_frozen_{scale_label(scale)}"
    )
...
def experiment_name(svd_dim: int, scale: float) -> str:
    return (
        f"stage4_{CONTEXT_METHOD}_i{IMAGE_WINDOW}_"
        f"{IMAGE_SPEC}_r{RETURN_HORIZON}_c{CONTEXT_WINDOW}_{experiment_suffix(svd_dim, scale)}"
    )
...
def assert_required_code():
    ...
    marker_checks = [
        ...
        ("src/stage4_film/runners/context_experiment.py", "film_full_confidence_gated_last_block"),
        ("src/stage4_film/runners/context_experiment.py", "configure_stage2_pretrained_context_model"),
        ("src/stage4_film/models/film_stock_cnn.py", "ConfidenceGatedLastBlockFilmContextStockCNN"),
        ("src/stage4_film/models/film_stock_cnn.py", "torch.abs(2.0 * stage2_prob_up - 1.0)"),
...
def patch_prebuilt_news_config(
    svd_dim: int,
    scale: float,
    ...
    cfg["stage4_model"]["experiment_suffix"] = experiment_suffix(svd_dim, scale)
    cfg["stage4_model"]["context_methods"] = [CONTEXT_METHOD]
    cfg["stage4_model"].setdefault("film_full_confidence_gated_last_block", {})
    cfg["stage4_model"]["film_full_confidence_gated_last_block"]["modulation_scale"] = float(scale)
...
def minimal_completed(svd_dim: int, scale: float, run_seed: int) -> bool:
    """Check metric-level completion when Grad-CAM is disabled."""

    exp = experiment_name(svd_dim, scale)
...
def output_check_cmd(svd_dim: int, scale: float, run_seed: int):
    return [
        sys.executable, "-u",
        "scripts/check_stage4_outputs.py",
        "--config", "configs/env_kaggle.yaml",
...
def is_completed(svd_dim: int, scale: float, run_seed: int) -> bool:
    if not RUN_GRADCAM:
        return minimal_completed(svd_dim, scale, run_seed)
    result = run(output_check_cmd(svd_dim, scale, run_seed), capture=True, check=False)
    return result.returncode == 0


def read_result_row(svd_dim: int, scale: float, run_seed: int, status: str, error: str = "") -> dict:
    exp = experiment_name(svd_dim, scale)
    seed_dir = f"seed_{run_seed}"
    ...
    manifest_path = PROJECT_ROOT / "outputs/stage4/run_manifests" / exp / seed_dir / "run_manifest.json"
    row = {
        "n12_variant": N12B_GRID_LABEL,
        "svd_dim": svd_dim,
...
def summarize_seed_results(seed_rows: list[dict]) -> pd.DataFrame:
    ...
    rows = []
    for (variant, svd_dim, method, scale, freeze_classifier), frame in df.groupby(
        ["n12_variant", "svd_dim", "context_method", "modulation_scale", "freeze_classifier"],
        dropna=False,
    ):
        row = {
            "n12_variant": variant,
...
def write_result_bundle(seed_csv: Path, mean_std_csv: Path, run_summary_json: Path):
    ...
    include_paths.extend(sorted(tables.glob("stage4_n12b*")))
    include_paths.extend(sorted(tables.glob(f"stage4_{N12B_GRID_LABEL}*")))
    ...
    include_paths.extend(sorted((PROJECT_ROOT / "reports/figures/gradcam").glob("*pretrained_frozen_s*")))

    for svd_dim in SVD_DIMS:
        for scale in MODULATION_SCALES:
            if should_skip_grid_point(svd_dim, scale):
                continue
            exp = experiment_name(svd_dim, scale)
    ...
    with zipfile.ZipFile(BUNDLE_PATH, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        manifest = {
            ...
            "n12_variant": N12B_GRID_LABEL,
            "svd_dims": SVD_DIMS,
            "modulation_scales": MODULATION_SCALES,
            "skipped_grid_points": [list(item) for item in sorted(SKIP_GRID_POINTS)],
            "run_seeds": RUN_SEEDS,
            "context_method": CONTEXT_METHOD,
            "freeze_classifier": FREEZE_CLASSIFIER,
...
print(f"Stage 2 checkpoint bundle: {checkpoint_bundle}", flush=True)
...
# ============================================================
# 3. Train/evaluate news-conditioned bounded FiLM over SVD/scale grid
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
for svd_dim in SVD_DIMS:
    ...
    for scale in MODULATION_SCALES:
        if should_skip_grid_point(svd_dim, scale):
            print(
                f"[skip-grid] SVD dim {svd_dim}, scale {scale} is marked as completed",
                flush=True,
            )
            continue
        patch_prebuilt_news_config(svd_dim, scale, feature_order, checkpoint_bundle)
        ...
        for run_seed in RUN_SEEDS:
            ...
            if SKIP_COMPLETED and is_completed(svd_dim, scale, run_seed):
                print(f"[skip] Metric output already exists for {exp}, seed={run_seed}", flush=True)
                summary_rows.append(read_result_row(svd_dim, scale, run_seed, status="skipped_completed"))
                continue
            ...
            try:
                ...
                if RUN_GRADCAM:
                    ...
                    run(output_check_cmd(svd_dim, scale, run_seed))

                summary_rows.append(read_result_row(svd_dim, scale, run_seed, status="ok"))
            except Exception as exc:
                print(f"[error] {exp}, seed={run_seed}: {exc}", flush=True)
                summary_rows.append(read_result_row(svd_dim, scale, run_seed, status="failed", error=str(exc)))
...
display(Markdown("# Stage 4 N12-B Confidence-Gated News FiLM Seed-Level Results"))
display(seed_df.sort_values(["svd_dim", "modulation_scale", "run_seed"]))

display(Markdown("# Stage 4 N12-B Confidence-Gated News FiLM Mean/Std Summary"))
if not mean_std_df.empty and "accuracy_mean" in mean_std_df.columns:
    display(mean_std_df.sort_values(["accuracy_mean", "roc_auc_mean"], ascending=[False, False]))
else:
    display(mean_std_df)
...
display(pd.DataFrame([
    {
        "reference": "N10 news SVD32 pretrained/frozen scale0.02",
        "context_dim": 102,
        "accuracy_mean": 0.5809,
        "roc_auc_mean": 0.5863,
        "note": "Closest news-only bounded FiLM comparison before confidence gating.",
    },
    {
        "reference": "N8-B F&G-only pretrained/frozen scale0.02",
        "context_dim": 4,
        "accuracy_mean": 0.5803,
        "roc_auc_mean": 0.5849,
        "note": "Closest current baseline-preserving context-FiLM result.",
    },
    {
    ...
    },
    {
        ...
        "roc_auc_mean": 0.5849,
        "note": "Target baseline before N12-B.",
    },
]))
