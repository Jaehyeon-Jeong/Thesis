# Kaggle Stage 4 N13-4 RORO-Proxy-Only Pretrained Frozen Bounded FiLM One Cell
...
- Stage 4 code snapshot, preferably the combined zip with the Stage 2
  ...
  `/kaggle/input/datasets/moskow/stage4/stage4_film_conditioning_n13_4_with_stage2_bundle.zip`
...
This runner executes `4-N13-4` with a compact RORO feature-set grid:

```text
cached public VIX/S&P500/DXY/US10Y CSV files
-> train-only PCA/SVD risk-off proxy
-> 20/60-observation RORO rolling features
-> prebuilt RORO context per seed
-> filtered proxy-only feature-set contexts: roro_2 / roro_3 / roro_proxy_all
-> Stage 2 I60/R20/ohlc_ma_vb checkpoint loaded/frozen
-> train only RORO context encoder + bounded final-block FiLM heads
...
CODE_INPUT_CANDIDATES = [
    Path("/kaggle/input/datasets/moskow/stage4/stage4_film_conditioning_n13_4_with_stage2_bundle.zip"),
...
SOURCE_CONTEXT_FEATURE_SET = "public_roro_pca_lag1_w20_60"
SOURCE_CONTEXT_NAME = (
    f"stage4_roro_context_i{IMAGE_WINDOW}_{IMAGE_SPEC}_"
...
RORO_FEATURE_SETS = {
    # Keep this first N13-4 run proxy-only. Raw components remain in the source
    # artifact for audit, but N13-4 tests whether the compact PCA proxy is useful.
    "roro_2": ["roro_proxy_mean_60", "roro_proxy_delta_60"],
    "roro_3": ["roro_proxy_mean_60", "roro_proxy_delta_60", "roro_proxy_std_60"],
    "roro_proxy_all": list(ALL_RORO_FEATURES),
}
CONTEXT_FEATURE_SET_KEYS = ["roro_2", "roro_3", "roro_proxy_all"]
...
# Smoke check only. For the real N13-4 run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_n13_4_roro_only_pretrained_frozen_bounded_film"
BUNDLE_PATH = Path("/kaggle/working/stage4_n13_4_roro_only_pretrained_frozen_bounded_film_result_bundle.zip")
...
def selected_context_features(feature_set_key: str) -> list[str]:
    if feature_set_key not in RORO_FEATURE_SETS:
        raise KeyError(f"Unknown RORO feature set: {feature_set_key}")
    return list(RORO_FEATURE_SETS[feature_set_key])
...
def experiment_suffix(scale: float, feature_set_key: str) -> str:
    return f"{feature_set_key}_public_roro_pretrained_frozen_{scale_label(scale)}"
...
def assert_required_code():
    ...
    marker_checks = [
        ...
        ("scripts/build_stage4_roro_context_features.py", "riskoff_dollar_return_20"),
...
def patch_base_config(checkpoint_bundle: Path):
    ...
    cfg["context"]["primary_features"] = list(ALL_RORO_FEATURES)
    cfg["context"]["normalized_feature_columns"] = [f"{feature}_normalized" for feature in ALL_RORO_FEATURES]
    ...
    cfg["stage4_model"]["context_dim"] = len(ALL_RORO_FEATURES)
...
def patch_prebuilt_roro_config(scale: float, feature_set_key: str, checkpoint_bundle: Path):
    config_path = PROJECT_ROOT / "configs" / "env_kaggle.yaml"
    cfg = yaml.safe_load(config_path.read_text(encoding="utf-8"))
...
def build_roro_context_for_seed(run_seed: int):
    ...
    if scaler.exists() and features.exists():
        print(f"Source RORO context exists for seed {run_seed}: {source_context_seed_dir(run_seed)}", flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_roro_context_features.py",
...
def prepare_roro_feature_subset_context(run_seed: int, feature_set_key: str):
    """Create a prebuilt context folder whose scaler exposes only one RORO subset."""
    ...
    if not source_context.exists() or not source_scaler.exists():
        raise FileNotFoundError(f"Source RORO context is missing for seed {run_seed}: {src_dir}")

    table = pd.read_csv(source_context)
    required = {"split", "sample_index", *features, *normalized}
    missing = sorted(required.difference(table.columns))
    if missing:
        raise KeyError(f"Source RORO context missing subset column(s): {missing}")
    ...
    if "feature_groups" in scaler:
        scaler["feature_groups"] = {"public_roro_selected": features}
...
# ============================================================
# 2. Build RORO context artifacts per seed
# ============================================================
patch_base_config(checkpoint_bundle)
for run_seed in RUN_SEEDS:
    print("\n" + "#" * 80, flush=True)
    print(f"Build Public RORO context for seed {run_seed}", flush=True)
    print("#" * 80, flush=True)
    build_roro_context_for_seed(run_seed)
    for feature_set_key in CONTEXT_FEATURE_SET_KEYS:
        prepare_roro_feature_subset_context(run_seed, feature_set_key)


# ============================================================
# 3. Train/evaluate RORO-only frozen bounded FiLM
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
for run_seed in RUN_SEEDS:
    for feature_set_key in CONTEXT_FEATURE_SET_KEYS:
        for scale in MODULATION_SCALES:
            patch_prebuilt_roro_config(scale, feature_set_key, checkpoint_bundle)
            exp = experiment_name(scale, feature_set_key)
            print("\n" + "=" * 80, flush=True)
...
display(Markdown("# Stage 4 N13-4 RORO-Proxy-Only Seed-Level Results"))
display(seed_df.sort_values(["context_feature_set_key", "modulation_scale", "run_seed"]))

display(Markdown("# Stage 4 N13-4 RORO-Proxy-Only Mean/Std Summary"))
