# Kaggle Stage 4 N13-5B Selected-Combo Pretrained Frozen Bounded FiLM One Cell

Copy the Python cell below into Kaggle after attaching:

- Stage 4 code snapshot, preferably the combined zip with the Stage 2
  checkpoint bundle embedded:
  `/kaggle/input/datasets/moskow/stage4/stage4_film_conditioning_n13_5b_with_stage2_bundle.zip`
...
This runner executes `4-N13-5B`:

```text
F&G/technical context + news TF-IDF/SVD32 + OFR FSI + public RORO
-> N13-5A-selected compact six-feature context
...
The selected feature vector is intentionally small:
...
The default modulation scale is conservative: `0.02`.
...
CODE_INPUT_CANDIDATES = [
    Path("/kaggle/input/datasets/moskow/stage4/stage4_film_conditioning_n13_5b_with_stage2_bundle.zip"),
...
FEATURE_SET_NAME = "n13_5a_combo6"
SELECTED_CONTEXT_PREFIX = "stage4_selected_combo_context"
NEWS_WINDOWS = [7, 20, 60]
NEWS_SVD_DIM = 32
NEWS_TFIDF_PREFIX = "stage4_news_tfidf_svd"
NEWS_FEATURE_SET = "tfidf_svd_w7_20_60"
FSI_FEATURE_SET = "ofr_fsi_lag1_w20_60"
RORO_FEATURE_SET = "public_roro_pca_lag1_w20_60"

SELECTED_FEATURES = [
    "combo_news_svd_60d_09",
    "combo_news_svd_20d_18",
    "combo_fg_mean_60",
    "combo_fg_delta_60",
    "combo_ofr_fsi_std_60",
    "combo_riskoff_dollar_return_20",
]
...
# N13-6 handles targeted interpretation. Keep False for this metric run.
RUN_GRADCAM = False
GRADCAM_SAMPLES_PER_CLASS = 2

# Smoke check only. For the real N13-5B run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_n13_5b_selected_combo_pretrained_frozen_bounded_film"
BUNDLE_PATH = Path("/kaggle/working/stage4_n13_5b_selected_combo_pretrained_frozen_bounded_film_result_bundle.zip")
...
def selected_context_name() -> str:
    return (
        f"{SELECTED_CONTEXT_PREFIX}_i{IMAGE_WINDOW}_{IMAGE_SPEC}_"
        f"r{RETURN_HORIZON}_{FEATURE_SET_NAME}"
    )


def structured_context_name() -> str:
    return f"stage4_context_i{IMAGE_WINDOW}_{IMAGE_SPEC}_r{RETURN_HORIZON}_c{CONTEXT_WINDOW}"


def news_context_name() -> str:
    return f"stage4_news_context_i{IMAGE_WINDOW}_{IMAGE_SPEC}_r{RETURN_HORIZON}_{NEWS_FEATURE_SET}"


def fsi_context_name() -> str:
    return f"stage4_fsi_context_i{IMAGE_WINDOW}_{IMAGE_SPEC}_r{RETURN_HORIZON}_{FSI_FEATURE_SET}"


def roro_context_name() -> str:
    return f"stage4_roro_context_i{IMAGE_WINDOW}_{IMAGE_SPEC}_r{RETURN_HORIZON}_{RORO_FEATURE_SET}"


def selected_context_seed_dir(run_seed: int) -> Path:
    return PROJECT_ROOT / "outputs/stage4/context" / selected_context_name() / f"seed_{run_seed}"


def experiment_suffix(scale: float) -> str:
    return f"{FEATURE_SET_NAME}_selected_combo_pretrained_frozen_{scale_label(scale)}"
...
def find_stage2_checkpoint_bundle() -> Path:
    ...
    candidates = [
        PROJECT_ROOT / "stage2_i60_ohlc_ma_vb_r20_seed42_46_checkpoints_for_stage4_n8",
        DATA_ROOT / "datasets/moskow/stage4/stage4_film_conditioning/stage2_i60_ohlc_ma_vb_r20_seed42_46_checkpoints_for_stage4_n8",
        DATA_ROOT / "datasets/moskow/stage4/stage2_i60_ohlc_ma_vb_r20_seed42_46_checkpoints_for_stage4_n8",
        DATA_ROOT / "datasets/moskow/stage2/stage2_i60_ohlc_ma_vb_r20_seed42_46_checkpoints_for_stage4_n8",
    ]
    candidates.extend(DATA_ROOT.rglob("stage2_i60_ohlc_ma_vb_r20_seed42_46_checkpoints_for_stage4_n8"))
    candidates.extend(DATA_ROOT.rglob("stage2_i60_ohlc_ma_vb_r20_seed42_46_checkpoints_for_stage4_n8.zip"))
    ...
    for candidate in candidates:
        try:
            ...
            print(f"[checkpoint-candidate-skip] {candidate}: {exc}", flush=True)

    raise FileNotFoundError(
        "Could not auto-detect Stage 2 checkpoint bundle. Attach the N8-A0 bundle "
...
def assert_required_code():
    required_stage4 = [
        ...
        "scripts/build_stage4_selected_combo_context_features.py",
    ...
    marker_checks = [
        ("scripts/build_stage4_selected_combo_context_features.py", "N13-5B selected-combo context feature builder"),
...
def patch_selected_combo_config(scale: float, checkpoint_bundle: Path):
    config_path = PROJECT_ROOT / "configs" / "env_kaggle.yaml"
    cfg = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    ...
    cfg["context"]["prebuilt_context_name"] = selected_context_name()
    cfg["context"]["primary_features"] = list(SELECTED_FEATURES)
    cfg["context"]["normalized_feature_columns"] = [f"{feature}_normalized" for feature in SELECTED_FEATURES]

    cfg["stage4_model"]["context_dim"] = len(SELECTED_FEATURES)
    ...
    config_path.write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")
    print("Selected-combo config patched:", config_path, "scale=", scale, flush=True)
...
def ensure_headline_windows():
    ...
    if NEWS_CSV and not Path(NEWS_CSV).exists():
        print(f"[warning] local NEWS_CSV not found, trying default news loader: {NEWS_CSV}", flush=True)
...
def ensure_news_vectors():
    output_path = (
        PROJECT_ROOT
        / f"outputs/stage4/news/{NEWS_TFIDF_PREFIX}_i{IMAGE_WINDOW}_r{RETURN_HORIZON}"
        / "news_tfidf_svd_features.parquet"
    )
    if output_path.exists():
        print("News TF-IDF/SVD vectors already exist:", output_path, flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_news_tfidf_svd.py",
        ...
        "--return-horizon", str(RETURN_HORIZON),
        "--window-days", *map(str, NEWS_WINDOWS),
        "--svd-dim", str(NEWS_SVD_DIM),
        "--output-prefix", NEWS_TFIDF_PREFIX,
    ])


def build_structured_context_for_seed(run_seed: int):
    output_dir = PROJECT_ROOT / "outputs/stage4/context" / structured_context_name() / f"seed_{run_seed}"
    if (output_dir / "context_features.csv").exists() and (output_dir / "context_scaler.json").exists():
        print("Structured context exists:", output_dir, flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_context_features.py",
...
def build_news_context_for_seed(run_seed: int):
    output_dir = PROJECT_ROOT / "outputs/stage4/context" / news_context_name() / f"seed_{run_seed}"
    if (output_dir / "context_features.csv").exists() and (output_dir / "context_scaler.json").exists():
        print("News context exists:", output_dir, flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_news_context_features.py",
        ...
        "--run-seed", str(run_seed),
        "--window-days", *map(str, NEWS_WINDOWS),
        "--input-prefix", NEWS_TFIDF_PREFIX,
        "--feature-set-name", NEWS_FEATURE_SET,
        "--write-report-copy",
    ])


def build_fsi_context_for_seed(run_seed: int):
    output_dir = PROJECT_ROOT / "outputs/stage4/context" / fsi_context_name() / f"seed_{run_seed}"
    if (output_dir / "context_features.csv").exists() and (output_dir / "context_scaler.json").exists():
        print("FSI context exists:", output_dir, flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_fsi_context_features.py",
        ...
        "--return-horizon", str(RETURN_HORIZON),
        "--run-seed", str(run_seed),
        "--fsi-csv", "data_inventory/ofr_fsi/fsi.csv",
        "--asof-lag-days", "1",
        "--output-prefix", "stage4_fsi_context",
        "--feature-set-name", FSI_FEATURE_SET,
        "--write-report-copy",
    ])


def build_roro_context_for_seed(run_seed: int):
    output_dir = PROJECT_ROOT / "outputs/stage4/context" / roro_context_name() / f"seed_{run_seed}"
    if (output_dir / "context_features.csv").exists() and (output_dir / "context_scaler.json").exists():
        print("RORO context exists:", output_dir, flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_roro_context_features.py",
        ...
        "--output-prefix", "stage4_roro_context",
        "--feature-set-name", RORO_FEATURE_SET,
        "--write-report-copy",
    ])


def build_selected_combo_context_for_seed(run_seed: int):
    output_dir = selected_context_seed_dir(run_seed)
    if (output_dir / "context_features.csv").exists() and (output_dir / "context_scaler.json").exists():
        print("Selected-combo context exists:", output_dir, flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_selected_combo_context_features.py",
        ...
        "--output-prefix", SELECTED_CONTEXT_PREFIX,
        "--write-report-copy",
    ])
...
def read_result_row(scale: float, run_seed: int, checkpoint_bundle: Path, status: str, error: str = "") -> dict:
    ...
    row = {
        ...
        "context_feature_set": FEATURE_SET_NAME,
        "context_features": ",".join(SELECTED_FEATURES),
        "context_dim": len(SELECTED_FEATURES),
...
def make_result_bundle():
    include_roots = [
        ...
        PROJECT_ROOT / "outputs/stage4/context" / selected_context_name(),
...
# ============================================================
# 2. Build source contexts and selected-combo prebuilt contexts
# ============================================================
...
for run_seed in RUN_SEEDS:
    print("\n" + "#" * 80, flush=True)
    print(f"Build selected-combo source contexts for seed {run_seed}", flush=True)
    print("#" * 80, flush=True)
    build_structured_context_for_seed(run_seed)
    build_news_context_for_seed(run_seed)
    build_fsi_context_for_seed(run_seed)
    build_roro_context_for_seed(run_seed)
    build_selected_combo_context_for_seed(run_seed)


# ============================================================
# 3. Train/evaluate selected-combo frozen bounded FiLM
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
for scale in MODULATION_SCALES:
    patch_selected_combo_config(scale, checkpoint_bundle)
    for run_seed in RUN_SEEDS:
        exp = experiment_name(scale)
        print("\n" + "=" * 90, flush=True)
        ...
        if SKIP_COMPLETED and is_completed(scale, run_seed):
            ...
            summary_rows.append(read_result_row(scale, run_seed, checkpoint_bundle, status="skipped_completed"))
            continue
        ...
        try:
            ...
            summary_rows.append(read_result_row(scale, run_seed, checkpoint_bundle, status="ok"))
        except Exception as exc:
            print(f"[error] {exp}, seed={run_seed}: {exc}", flush=True)
            summary_rows.append(read_result_row(scale, run_seed, checkpoint_bundle, status="failed", error=str(exc)))
            if not CONTINUE_ON_ERROR:
                raise


# ============================================================
# 4. Write summaries and bundle outputs
# ============================================================
...
display(Markdown("# Stage 4 N13-5B Selected-Combo Seed-Level Results"))
display(seed_df.sort_values(["modulation_scale", "run_seed"]))

display(Markdown("# Stage 4 N13-5B Selected-Combo Mean/Std Summary"))
