# Kaggle Stage 4 N13-7A F&G Bounded FiLM Scale Grid One Cell
...
This runner executes 4-N13-7A:

```text
Stage 2 I60/R20/ohlc_ma_vb checkpoint -> load learned weights
freeze visual CNN blocks + freeze classifier
train only F&G context encoder + bounded final-block FiLM heads
context: fg_value, fg_mean_60, fg_delta_60, fg_std_60
scale grid: 0.10, 0.20
seeds: 42, 43, 44, 45, 46
```

Purpose:
- N8-B already tested small bounded scales `0.02` and `0.05`.
- N13-7A keeps the same Stage 2 frozen setup and tests whether the current FiLM
  branch was too conservative by allowing larger gamma/beta movement.
- This cell also writes Stage2-vs-FiLM correction/regression tables for each
  scale, so collapse and net correction are checked together with accuracy.
...
# Smoke check only. For the real N13-7A run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_n13_7a_fg_bounded_scale_grid"
BUNDLE_PATH = Path("/kaggle/working/stage4_n13_7a_fg_bounded_scale_grid_result_bundle.zip")
...
def experiment_suffix(scale: float) -> str:
    return f"n13_7a_fg_pretrained_frozen_{scale_label(scale)}"
...
def patch_config(scale: float, checkpoint_bundle: Path):
    """Patch Stage 4 Kaggle config for one N13-7A scale."""
...
def make_result_bundle():
    """Zip compact N13-7A outputs for local download."""
...
def run_correction_analysis(scale: float, checkpoint_bundle: Path):
    """Compare Stage 2 baseline predictions against one N13-7A FiLM scale."""
    ...
    run([
        ...
        "--analysis-name", f"N13-7A F&G bounded FiLM {scale_label(scale)}",
    ])
...
run([
    ...
    "--image-window", str(IMAGE_WINDOW),
    "--return-horizon", str(RETURN_HORIZON),
    "--output", "reports/tables/stage4_n13_7a_context_source_audit.json",
])
...
display(Markdown("# Stage 4 N13-7A Seed-Level Results"))
display(seed_df.sort_values(["modulation_scale", "run_seed"]))

display(Markdown("# Stage 4 N13-7A Mean/Std Summary"))
...
display(Markdown("# Stage 4 N13-7A Stage2-vs-FiLM Correction Summary"))
