# Kaggle Stage 4 N13-7D F&G Classifier-Unfreeze FiLM One Cell
...
- Stage 2 BTC code snapshot:
  `/kaggle/input/datasets/moskow/stage22/stage2_btc_extension`
...
This runner executes 4-N13-7D:

```text
Stage 2 I60/R20/ohlc_ma_vb checkpoint -> load learned weights
freeze visual CNN blocks
unfreeze final classifier
train classifier + F&G context encoder + bounded final-block FiLM heads
context: fg_value, fg_mean_60, fg_delta_60, fg_std_60
scale: 0.02
seeds: 42, 43, 44, 45, 46
```

Purpose:
- N8-B froze both the Stage 2 visual CNN and the Stage 2 classifier.
- N13-7A showed that simply increasing the bounded FiLM scale does not help.
- N13-7D keeps the visual CNN frozen but lets the final classifier adapt, so the
  learned context-FiLM correction can adjust the decision boundary without
  relearning chart filters.
- This cell also writes Stage2-vs-FiLM correction/regression tables for each
  seed, so collapse and net correction are checked together with accuracy.
...
STAGE2_CODE_INPUT = Path("/kaggle/input/datasets/moskow/stage22/stage2_btc_extension")
...
# Smoke check only. For the real N13-7D run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_n13_7d_fg_classifier_unfreeze"
BUNDLE_PATH = Path("/kaggle/working/stage4_n13_7d_fg_classifier_unfreeze_result_bundle.zip")
...
def experiment_suffix(scale: float) -> str:
    return f"n13_7d_fg_classifier_unfreeze_{scale_label(scale)}"
...
def patch_config(scale: float, checkpoint_bundle: Path):
    """Patch Stage 4 Kaggle config for one N13-7D classifier-unfreeze run."""
...
def make_result_bundle():
    """Zip compact N13-7D outputs for local download."""
...
def stage2_output_root_for_analysis(checkpoint_bundle: Path) -> Path:
    """Return the Stage 2 output root that contains baseline predictions."""
    ...
    raise FileNotFoundError(
        ...
        "Attach the N8-A0 Stage 2 bundle that includes outputs/stage2/predictions."
    )


def run_correction_analysis(scale: float, checkpoint_bundle: Path):
    """Compare Stage 2 baseline predictions against the N13-7D FiLM run."""

    stage2_output_root = stage2_output_root_for_analysis(checkpoint_bundle)
    prefix = f"{OUTPUT_PREFIX}_{scale_label(scale)}_stage2_vs_film_correction_analysis"
    run([
        sys.executable, "-u",
        "scripts/analyze_stage4_stage2_context_corrections.py",
        "--config", "configs/env_kaggle.yaml",
        ...
        "--context-method", CONTEXT_METHOD,
        "--stage4-experiment-suffix", experiment_suffix(scale),
        ...
        "--analysis-name", f"N13-7D F&G classifier-unfreeze {scale_label(scale)}",
    ])


def summarize_corrections(scales: list[float]) -> pd.DataFrame:
    """Read correction-analysis seed summaries into one compact table."""

    rows = []
    tables_root = PROJECT_ROOT / "reports/tables"
    for scale in scales:
        prefix = f"{OUTPUT_PREFIX}_{scale_label(scale)}_stage2_vs_film_correction_analysis"
        path = tables_root / f"{prefix}_seed_summary.csv"
        if not path.exists():
            rows.append({
                "modulation_scale": float(scale),
                "correction_summary_available": False,
            })
            continue
        ...
        numeric_columns = [
            ...
            "stage2_accuracy",
            "stage4_accuracy",
            "accuracy_delta_stage4_minus_stage2",
            "stage2_wrong_to_stage4_correct",
            "stage2_correct_to_stage4_wrong",
            "stage2_predicted_positive_rate",
            "stage4_predicted_positive_rate",
            "mean_true_prob_delta_stage4_minus_stage2",
        ]
        row = {
            "modulation_scale": float(scale),
            "correction_summary_available": True,
            "seed_count": int(frame["run_seed"].nunique()) if "run_seed" in frame.columns else len(frame),
        }
        for column in numeric_columns:
            if column in frame.columns:
                values = pd.to_numeric(frame[column], errors="coerce")
                row[f"{column}_mean"] = values.mean()
                row[f"{column}_sum"] = values.sum()
        if {
            "stage2_wrong_to_stage4_correct",
            "stage2_correct_to_stage4_wrong",
        }.issubset(frame.columns):
            corrections = pd.to_numeric(frame["stage2_wrong_to_stage4_correct"], errors="coerce")
            regressions = pd.to_numeric(frame["stage2_correct_to_stage4_wrong"], errors="coerce")
            row["net_correction_sum"] = float(corrections.sum() - regressions.sum())
            row["net_correction_mean"] = float((corrections - regressions).mean())
        rows.append(row)
    return pd.DataFrame(rows)
...
run([
    ...
    "--image-window", str(IMAGE_WINDOW),
    "--return-horizon", str(RETURN_HORIZON),
    "--output", "reports/tables/stage4_n13_7d_context_source_audit.json",
])
...
for run_seed in RUN_SEEDS:
    print("\n" + "#" * 80, flush=True)
    print(f"Build F&G context for seed {run_seed}", flush=True)
    print("#" * 80, flush=True)
    patch_config(MODULATION_SCALES[0], checkpoint_bundle)
    run([
        sys.executable, "-u",
...
for scale in MODULATION_SCALES:
    patch_config(scale, checkpoint_bundle)
    try:
        run_correction_analysis(scale, checkpoint_bundle)
    except Exception as exc:
        message = f"{scale_label(scale)} correction analysis failed: {exc}"
        print("[warning]", message, flush=True)
        correction_errors.append(message)
...
correction_df = summarize_corrections(MODULATION_SCALES)

seed_csv = tables_root / f"{OUTPUT_PREFIX}_seed_results.csv"
mean_std_csv = tables_root / f"{OUTPUT_PREFIX}_mean_std_results.csv"
correction_csv = tables_root / f"{OUTPUT_PREFIX}_correction_summary.csv"
...
if correction_errors:
    (tables_root / f"{OUTPUT_PREFIX}_correction_errors.json").write_text(
        json.dumps(correction_errors, indent=2),
        encoding="utf-8",
    )
make_result_bundle()

display(Markdown("# Stage 4 N13-7D Seed-Level Results"))
display(seed_df.sort_values(["modulation_scale", "run_seed"]))

display(Markdown("# Stage 4 N13-7D Mean/Std Summary"))
...
display(Markdown("# Stage 4 N13-7D Stage2-vs-FiLM Correction Summary"))
display(correction_df.sort_values("modulation_scale"))

print("\nDONE", flush=True)
print("Seed CSV:", seed_csv, flush=True)
print("Mean/std CSV:", mean_std_csv, flush=True)
print("Correction summary CSV:", correction_csv, flush=True)
