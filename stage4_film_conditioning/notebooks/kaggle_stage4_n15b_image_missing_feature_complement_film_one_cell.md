# Kaggle Stage 4 N15-B Image-Missing-Feature Complement FiLM One Cell
...
for each image spec in ohlc / ohlc_ma / ohlc_vb / ohlc_ma_vb:
    ...
    use feature set selected to complement what the image omits
```
...
```text
Can context-FiLM recover information that was not drawn into the chart image,
without relearning the Stage 2 chart filters?
```
...
CODE_INPUT_CANDIDATES = [
    ...
    Path("/kaggle/input/datasets/moskow/stage4/stage4_film_conditioning_n15b_snapshot.zip"),
]
...
# Leave empty to auto-detect. If auto-detect fails, set this to a folder or zip
# that contains:
...
SPEC_FEATURE_ROWS = [
    {
        "image_spec": "ohlc",
        "feature_set": "technical_all",
        "features": ["bb_percent_b_60", "bb_bandwidth_60", "mfi_60", "rv_60"],
        "rationale": "OHLC image omits MA/VB information; add compact technical context.",
    },
    {
        "image_spec": "ohlc_ma",
        "feature_set": "volume_volatility",
        "features": ["mfi_60", "rv_60"],
        "rationale": "OHLC+MA image omits volume bars; add volume-aware MFI and realized volatility.",
    },
    {
        "image_spec": "ohlc_vb",
        "feature_set": "bb_trend",
        "features": ["bb_percent_b_60", "bb_bandwidth_60"],
        "rationale": "OHLC+VB image omits moving-average trend context; add Bollinger state.",
    },
    {
        "image_spec": "ohlc_ma_vb",
        "feature_set": "technical_all_control",
        "features": ["bb_percent_b_60", "bb_bandwidth_60", "mfi_60", "rv_60"],
        "rationale": "Full image control; technical context should be mostly redundant.",
    },
]
...
# Smoke check only. For the real N15-B run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_n15b_image_missing_feature_complement"
BUNDLE_PATH = Path("/kaggle/working/stage4_n15b_image_missing_feature_complement_result_bundle.zip")
...
def context_feature_set_name(row: dict) -> str:
    return f"n15b_{row['feature_set']}"
...
def experiment_suffix(row: dict) -> str:
    return f"n15b_{row['feature_set']}_pretrained_frozen_{scale_label(MODULATION_SCALE)}"
...
def patch_config(row: dict, checkpoint_bundle: Path):
    """Patch Stage 4 Kaggle config for one N15-B row."""
...
def run_correction_analysis(row: dict, checkpoint_bundle: Path):
    ...
    run([
        ...
        "--split", EVAL_SPLIT,
        "--top-k", "20",
        "--analysis-name", f"N15-B {row['image_spec']} {row['feature_set']}",
...
def make_result_bundle():
    """Zip N15-B outputs for local download and future Kaggle resume."""
...
# ============================================================
# 2. Train/evaluate/check N15-B rows
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
for row in SPEC_FEATURE_ROWS:
    print("\n" + "#" * 100, flush=True)
    print(f"N15-B row: image_spec={row['image_spec']} feature_set={row['feature_set']}", flush=True)
...
display(Markdown("# Stage 4 N15-B Seed-Level Results"))
if not seed_df.empty:
    display(seed_df.sort_values(["image_spec", "context_feature_set", "run_seed"]))
else:
    display(seed_df)

display(Markdown("# Stage 4 N15-B Mean/Std Summary"))
...
display(Markdown("# Stage 4 N15-B Correction Seed Summary"))
display(correction_seed_df)

display(Markdown("# Stage 4 N15-B Transition Summary"))
