# Kaggle Stage 4 N16-3 Derivatives Feature-Set Grid One Cell
...
- Stage 2 `I60/R20` checkpoint/prediction bundle for `ohlc_ma_vb`
- BTC OHLCV Kaggle dataset

This runner executes `4-N16-3` only:
...
and evaluates the four derivatives context feature sets.
...
# Smoke check only. For the real N16-3 run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_n16_3_derivatives_feature_set_grid"
BUNDLE_PATH = Path("/kaggle/working/stage4_n16_3_derivatives_feature_set_grid_result_bundle.zip")
...
def experiment_suffix(feature_set: str) -> str:
    return f"n16_{feature_set}_pretrained_frozen_{scale_label(MODULATION_SCALE)}"
...
def find_stage2_checkpoint_bundle() -> Path:
    ...
    raise FileNotFoundError(
        "Could not find Stage 2 I60/R20 ohlc_ma_vb checkpoint bundle. "
...
def run_correction_analysis(feature_set: str, checkpoint_bundle: Path):
    ...
    run([
        ...
        "--split", EVAL_SPLIT,
        "--top-k", "20",
        "--analysis-name", f"N16-3 {feature_set}",
...
# ============================================================
# 2. Train/evaluate/check N16-3 feature-set grid
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
for feature_set in FEATURE_SETS:
    print("\n" + "#" * 100, flush=True)
    print(f"N16-3 feature_set={feature_set}", flush=True)
...
display(Markdown("# Stage 4 N16-3 Seed-Level Results"))
if not seed_df.empty:
    display(seed_df.sort_values(["context_feature_set", "run_seed"]))
else:
    display(seed_df)

display(Markdown("# Stage 4 N16-3 Mean/Std Summary"))
...
display(Markdown("# Stage 4 N16-3 Correction Seed Summary"))
display(correction_seed_df)

display(Markdown("# Stage 4 N16-3 Transition Summary"))
