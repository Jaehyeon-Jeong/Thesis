# Kaggle Stage 4 N16-4 OHLC-VB Derivatives Repeat One Cell
...
- Stage 2 `I60/R20` checkpoint/prediction bundle containing `ohlc_vb`
- BTC OHLCV Kaggle dataset

This runner executes `4-N16-4` only:

```text
image spec: ohlc_vb
model: Stage2 pretrained visual CNN + classifier frozen
...
The cell rebuilds N16 prebuilt context artifacts if they are missing, then trains
and evaluates the two selected derivatives context feature sets.
...
# N15-A Stage 2 CHECKPOINT bundle. This is not the Stage 2 source-code folder.
# If this path does not exist in your Kaggle dataset, replace it with the folder
# or zip that contains:
# outputs/stage2/checkpoints/stage2_i60_ohlc_vb_r20/seed_42/best.pt
# ...
# outputs/stage2/checkpoints/stage2_i60_ohlc_vb_r20/seed_46/best.pt
...
# Smoke check only. For the real N16-4 run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_n16_4_ohlc_vb_derivatives_repeat"
BUNDLE_PATH = Path("/kaggle/working/stage4_n16_4_ohlc_vb_derivatives_repeat_result_bundle.zip")
...
def context_feature_set_name(feature_set: str) -> str:
    return f"n16d_{feature_set}"


def context_name(feature_set: str) -> str:
    return f"stage4_derivatives_context_i{IMAGE_WINDOW}_{IMAGE_SPEC}_r{RETURN_HORIZON}_{context_feature_set_name(feature_set)}"


def experiment_suffix(feature_set: str) -> str:
    return f"n16d_{feature_set}_pretrained_frozen_{scale_label(MODULATION_SCALE)}"


def experiment_name(feature_set: str) -> str:
    return (
        f"stage4_{CONTEXT_METHOD}_i{IMAGE_WINDOW}_{IMAGE_SPEC}_"
        f"r{RETURN_HORIZON}_c{CONTEXT_WINDOW}_{experiment_suffix(feature_set)}"
    )
...
def resolve_checkpoint_candidate(candidate: Path) -> Path:
    ...
    if candidate.is_file():
        ...
        extract_root = Path("/kaggle/working/stage2_n16_checkpoint_bundle_extracted") / candidate.stem
...
def find_stage2_checkpoint_bundle() -> Path:
    ...
    raise FileNotFoundError(
        "Could not find Stage 2 I60/R20 ohlc_vb checkpoint bundle. "
        "Attach the N15-A bundle dataset or set STAGE2_CHECKPOINT_BUNDLE manually."
    )
...
def assert_required_code():
    ...
    if missing_stage4:
        raise RuntimeError("Stage 4 snapshot is incomplete: " + ", ".join(missing_stage4))
    ...
    if stale:
        raise RuntimeError("Stage 4 snapshot is stale/incomplete. Problems: " + "; ".join(stale))
...
def context_root(feature_set: str, run_seed: int) -> Path:
    return PROJECT_ROOT / "outputs/stage4/context" / context_name(feature_set) / f"seed_{run_seed}"


def ensure_context_artifact(feature_set: str, run_seed: int):
    root = context_root(feature_set, run_seed)
    if (root / "context_features.csv").exists() and (root / "context_scaler.json").exists():
        return
    if not BUILD_CONTEXT_IF_MISSING:
        raise FileNotFoundError(f"Missing context artifact: {root}")
    run([
        sys.executable, "-u",
        "scripts/build_stage4_derivatives_context_features.py",
        ...
        "--feature-set-name", context_feature_set_name(feature_set),
        "--daily-asof-lag-days", str(DAILY_ASOF_LAG_DAYS),
    ] + (["--write-report-copy"] if int(run_seed) == 42 else []))


def feature_order(feature_set: str, run_seed: int) -> list[str]:
    path = context_root(feature_set, run_seed) / "context_scaler.json"
    payload = json.loads(path.read_text(encoding="utf-8"))
    return list(payload["feature_order"])


def patch_prebuilt_config(feature_set: str, run_seed: int, checkpoint_bundle: Path):
    config_path = PROJECT_ROOT / "configs" / "env_kaggle.yaml"
    cfg = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    features = feature_order(feature_set, run_seed)

    cfg["context"]["source"] = "prebuilt"
    cfg["context"]["context_window"] = CONTEXT_WINDOW
    cfg["context"]["feature_set_name"] = context_feature_set_name(feature_set)
    cfg["context"]["prebuilt_context_name"] = context_name(feature_set)
    ...
    cfg["stage4_model"]["experiment_suffix"] = experiment_suffix(feature_set)
...
def output_check_cmd(feature_set: str, run_seed: int):
    cmd = [
        sys.executable, "-u",
        "scripts/check_stage4_outputs.py",
        "--config", "configs/env_kaggle.yaml",
...
def is_completed(feature_set: str, run_seed: int, checkpoint_bundle: Path) -> bool:
    patch_prebuilt_config(feature_set, run_seed, checkpoint_bundle)
    result = run(output_check_cmd(feature_set, run_seed), capture=True, check=False)
    return result.returncode == 0


def read_result_row(feature_set: str, run_seed: int, status: str, error: str = "") -> dict:
    exp = experiment_name(feature_set)
    ...
    row = {
        ...
        "context_name": context_name(feature_set),
        "context_dim": len(feature_order(feature_set, run_seed)) if (context_root(feature_set, run_seed) / "context_scaler.json").exists() else None,
    ...
    if metrics_path.exists():
        ...
        if row.get("accuracy") is not None and row.get("majority_class_accuracy") is not None:
            row["accuracy_minus_majority_class_accuracy"] = float(row["accuracy"]) - float(row["majority_class_accuracy"])
    ...
    if manifest_path.exists():
        ...
        row["freeze_visual_backbone"] = pretrained.get("freeze_visual_backbone")
        row["freeze_classifier"] = pretrained.get("freeze_classifier")
...
def read_stage2_reference(checkpoint_bundle: Path) -> pd.DataFrame:
    candidates = [
        checkpoint_bundle / "reports/tables/stage2_n15a_i60_r20_four_image_specs_five_seed_mean_std_results.csv",
        checkpoint_bundle / "reports/tables/stage2_i20_i60_r20_five_seed_mean_std_results.csv",
    ]
    for path in candidates:
        if path.exists():
            frame = pd.read_csv(path)
            frame = frame[
                (frame["image_window"] == IMAGE_WINDOW)
                & (frame["return_horizon"] == RETURN_HORIZON)
                & (frame["image_spec"] == IMAGE_SPEC)
            ].copy()
            keep = ["image_spec"]
            renames = {}
            for metric in ["accuracy", "roc_auc", "f1", "brier_score"]:
                ...
                if col in frame.columns:
                    keep.append(col)
                    renames[col] = f"stage2_{metric}_mean"
...
def summarize_seed_results(seed_rows: list[dict], checkpoint_bundle: Path) -> pd.DataFrame:
    ...
    rows = []
    for feature_set, frame in df.groupby("context_feature_set", dropna=False):
        row = {
            "image_spec": IMAGE_SPEC,
            "context_feature_set": feature_set,
            "modulation_scale": float(MODULATION_SCALE),
    ...
    ref = read_stage2_reference(checkpoint_bundle)
    if not ref.empty:
        summary = summary.merge(ref, on="image_spec", how="left")
        for metric in ["accuracy", "roc_auc", "f1", "brier_score"]:
            s4 = f"{metric}_mean"
            s2 = f"stage2_{metric}_mean"
            if s4 in summary.columns and s2 in summary.columns:
                summary[f"{metric}_minus_same_image_stage2"] = summary[s4] - summary[s2]
    return summary


def stage2_output_root_for_analysis(checkpoint_bundle: Path) -> Path:
    expected_prediction = (
        Path("predictions")
        / stage2_experiment_name()
        / "seed_42"
    ...
    for candidate in candidates:
        if (candidate / expected_prediction).exists():
            return candidate
    raise FileNotFoundError(f"Missing Stage 2 prediction for correction analysis: {expected_prediction}")


def run_correction_analysis(feature_set: str, checkpoint_bundle: Path):
    stage2_output_root = stage2_output_root_for_analysis(checkpoint_bundle)
    prefix = f"{OUTPUT_PREFIX}_{feature_set}_stage2_vs_film_correction"
    run([
        sys.executable, "-u",
        "scripts/analyze_stage4_stage2_context_corrections.py",
        "--config", "configs/env_kaggle.yaml",
        ...
        "--context-method", CONTEXT_METHOD,
        "--stage4-experiment-suffix", experiment_suffix(feature_set),
        "--context-name", context_name(feature_set),
        "--stage2-output-root", str(stage2_output_root),
        "--stage4-output-root", str(PROJECT_ROOT / "outputs/stage4"),
        "--run-seeds", *map(str, RUN_SEEDS),
        "--split", EVAL_SPLIT,
        "--top-k", "20",
        "--analysis-name", f"N16-4 {feature_set}",
...
def read_correction_outputs(paths_by_feature: list[dict]) -> tuple[pd.DataFrame, pd.DataFrame]:
    seed_frames = []
    transition_frames = []
    for item in paths_by_feature:
        feature_set = item["feature_set"]
        if item["seed_summary"].exists():
            frame = pd.read_csv(item["seed_summary"])
            frame.insert(0, "context_feature_set", feature_set)
            seed_frames.append(frame)
        if item["transition_summary"].exists():
            frame = pd.read_csv(item["transition_summary"])
            frame.insert(0, "context_feature_set", feature_set)
...
# ============================================================
# 2. Train/evaluate/check N16-4 feature-set grid
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
summary_rows = []
total_runs = len(FEATURE_SETS) * len(RUN_SEEDS)
run_index = 0

for feature_set in FEATURE_SETS:
    print("\n" + "#" * 100, flush=True)
    print(f"N16-4 feature_set={feature_set}", flush=True)
    print("context:", context_name(feature_set), flush=True)
    print("#" * 100, flush=True)

    for run_seed in RUN_SEEDS:
        ensure_context_artifact(feature_set, run_seed)
        patch_prebuilt_config(feature_set, run_seed, checkpoint_bundle)
        ...
        if SKIP_COMPLETED and is_completed(feature_set, run_seed, checkpoint_bundle):
            print(f"[skip] Output check already passes for {exp}, seed={run_seed}", flush=True)
            summary_rows.append(read_result_row(feature_set, run_seed, status="skipped_completed"))
            continue
        ...
        try:
            run([
                ...
                "--context-method", CONTEXT_METHOD,
                "--run-seed", str(run_seed),
                "--experiment-suffix", experiment_suffix(feature_set),
                "--context-feature-set-name", context_feature_set_name(feature_set),
            ...
            run(output_check_cmd(feature_set, run_seed))
            summary_rows.append(read_result_row(feature_set, run_seed, status="ok"))
        except Exception as exc:
            print(f"[error] {exp}, seed={run_seed}: {exc}", flush=True)
            summary_rows.append(read_result_row(feature_set, run_seed, status="failed", error=str(exc)))
...
correction_paths = []
for feature_set in FEATURE_SETS:
    patch_prebuilt_config(feature_set, RUN_SEEDS[0], checkpoint_bundle)
    try:
        correction_paths.append(run_correction_analysis(feature_set, checkpoint_bundle))
    except Exception as exc:
        print(f"[warning] correction analysis failed for {feature_set}: {exc}", flush=True)
        if not CONTINUE_ON_ERROR:
            raise
...
display(Markdown("# Stage 4 N16-4 Seed-Level Results"))
if not seed_df.empty:
    display(seed_df.sort_values(["context_feature_set", "run_seed"]))
else:
    display(seed_df)

display(Markdown("# Stage 4 N16-4 Mean/Std Summary"))
...
display(Markdown("# Stage 4 N16-4 Correction Seed Summary"))
display(correction_seed_df)

display(Markdown("# Stage 4 N16-4 Transition Summary"))
