# Kaggle Stage 4 N8-B F&G Pretrained Frozen Bounded FiLM One Cell
...
- N8-B tests the cleaner thesis question: can F&G context improve a strong
  pretrained Stage 2 visual model through bounded correction?
...
# Smoke check only. For the real N8-B run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_n8b_fg_only_pretrained_frozen_bounded_film"
BUNDLE_PATH = Path("/kaggle/working/stage4_n8b_fg_only_pretrained_frozen_bounded_film_result_bundle.zip")
...
def experiment_suffix(scale: float) -> str:
    return f"n8b_fg_pretrained_frozen_{scale_label(scale)}"
...
def patch_config(scale: float, checkpoint_bundle: Path):
    """Patch Stage 4 Kaggle config for one N8-B scale."""
...
def make_result_bundle():
    """Zip compact N8-B outputs for local download."""
...
run([
    ...
    "--image-window", str(IMAGE_WINDOW),
    "--return-horizon", str(RETURN_HORIZON),
    "--output", "reports/tables/stage4_n8b_context_source_audit.json",
])
...
display(Markdown("# Stage 4 N8-B Seed-Level Results"))
display(seed_df.sort_values(["modulation_scale", "run_seed"]))

display(Markdown("# Stage 4 N8-B Mean/Std Summary"))
