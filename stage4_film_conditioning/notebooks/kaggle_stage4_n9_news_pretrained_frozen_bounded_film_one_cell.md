# Kaggle Stage 4 N9 News Pretrained/Frozen Bounded FiLM One Cell
...
This runner executes one N9 news-conditioned pretrained/baseline-preserving
FiLM variant at a time. The default is N9-A:

```text
I60 / R20 / ohlc_ma_vb
headline windows: 7d + 20d + 60d
Stage 2 checkpoint -> load learned visual/classifier weights
news SVD8 context -> MLP -> bounded last-block FiLM
CNN frozen
classifier frozen
FiLM scale: 0.02
...
- N9 tests whether headline-news context can improve that stronger setup.

Variant switches:

```text
N9A: scale 0.02, classifier frozen
N9B: scale 0.05, classifier frozen
N9C: scale 0.02, classifier trainable
N9D: scale 0.05, classifier trainable
```
...
# Pick one N9 variant for this run. Re-run the same cell after changing this
# value if N9-A is stable and you want N9-B/C/D.
N9_VARIANT = "N9A"
N9_VARIANTS = {
    "N9A": {"scale": 0.02, "freeze_classifier": True, "label": "n9a_news_pretrained_frozen_s0p02"},
    "N9B": {"scale": 0.05, "freeze_classifier": True, "label": "n9b_news_pretrained_frozen_s0p05"},
    "N9C": {"scale": 0.02, "freeze_classifier": False, "label": "n9c_news_pretrained_classifier_trainable_s0p02"},
    "N9D": {"scale": 0.05, "freeze_classifier": False, "label": "n9d_news_pretrained_classifier_trainable_s0p05"},
}
if N9_VARIANT not in N9_VARIANTS:
    raise ValueError(f"Unsupported N9_VARIANT: {N9_VARIANT}")
N9_CONFIG = N9_VARIANTS[N9_VARIANT]
MODULATION_SCALE = float(N9_CONFIG["scale"])
FREEZE_CLASSIFIER = bool(N9_CONFIG["freeze_classifier"])
...
OUTPUT_PREFIX = f"stage4_{N9_CONFIG['label']}_five_seed"
BUNDLE_PATH = Path(f"/kaggle/working/stage4_{N9_CONFIG['label']}_result_bundle.zip")
...
def experiment_suffix(svd_dim: int) -> str:
    return f"news_tfidf_svd{int(svd_dim)}_w7_20_60_{N9_CONFIG['label']}"
...
def patch_prebuilt_news_config(svd_dim: int, feature_order: list[str], checkpoint_bundle: Path):
...
def write_result_bundle(seed_csv: Path, mean_std_csv: Path, run_summary_json: Path):
    ...
    include_paths.extend(sorted(tables.glob(f"stage4_{N9_CONFIG['label']}*")))
    ...
    include_paths.extend(sorted((PROJECT_ROOT / "reports/figures/gradcam").glob(f"*{N9_CONFIG['label']}*")))
...
for svd_dim in SVD_DIMS:
    ...
    patch_prebuilt_news_config(svd_dim, feature_order, checkpoint_bundle)
...
display(Markdown(f"# Stage 4 {N9_VARIANT} News Pretrained/Frozen FiLM Seed-Level Results"))
display(seed_df.sort_values(["svd_dim", "run_seed"]))

display(Markdown(f"# Stage 4 {N9_VARIANT} News Pretrained/Frozen FiLM Mean/Std Summary"))
