# Kaggle Stage 4 N9 News Pretrained/Frozen SVD/Scale Grid One Cell
...
This runner executes the N9 news-conditioned pretrained/baseline-preserving
FiLM SVD/scale grid. The already completed N9-A anchor is excluded from the
run grid:
...
news SVD context -> MLP -> bounded last-block FiLM
CNN frozen
classifier frozen
FiLM grid: SVD8/0.05, SVD16/0.02, SVD16/0.05, SVD32/0.02, SVD32/0.05
...
- N9-A showed stable but very small correction from SVD8/scale0.02.
- This grid tests whether the correction was too weak or the SVD bottleneck
  was too tight, without changing the frozen Stage 2 visual/classifier path.

The grid intentionally skips `SVD8 / scale0.02` because it has already been
run as N9-A.
...
SKIP_GRID_POINTS = {(8, 0.02)}
CONTEXT_METHOD = "film_full_bounded_last_block"

N9_GRID_LABEL = "n9_news_pretrained_frozen_svd_scale_grid"
...
# Smoke check only. For the real N9 run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = f"stage4_{N9_GRID_LABEL}"
BUNDLE_PATH = Path(f"/kaggle/working/stage4_{N9_GRID_LABEL}_result_bundle.zip")
...
def summarize_seed_results(seed_rows: list[dict]) -> pd.DataFrame:
    ...
    for (variant, svd_dim, method, scale, freeze_classifier), frame in df.groupby(
        ["n9_variant", "svd_dim", "context_method", "modulation_scale", "freeze_classifier"],
        dropna=False,
    ):
        row = {
            "n9_variant": variant,
...
def write_result_bundle(seed_csv: Path, mean_std_csv: Path, run_summary_json: Path):
    ...
    include_paths.extend(sorted(tables.glob("stage4_n9*")))
    include_paths.extend(sorted(tables.glob(f"stage4_{N9_GRID_LABEL}*")))
...
for svd_dim in SVD_DIMS:
    ...
    for scale in MODULATION_SCALES:
        if should_skip_grid_point(svd_dim, scale):
            print(
                f"[skip-grid] SVD dim {svd_dim}, scale {scale} already covered by N9-A",
                flush=True,
            )
            continue
...
display(Markdown("# Stage 4 N9 News Pretrained/Frozen SVD/Scale Grid Seed-Level Results"))
display(seed_df.sort_values(["svd_dim", "modulation_scale", "run_seed"]))

display(Markdown("# Stage 4 N9 News Pretrained/Frozen SVD/Scale Grid Mean/Std Summary"))
...
display(pd.DataFrame([
    {
        "reference": "N9-A news SVD8 pretrained/frozen scale0.02",
        "context_dim": 30,
        "accuracy_mean": 0.5795,
        "roc_auc_mean": 0.5857,
        "note": "Completed anchor; excluded from this grid.",
    },
    {
    ...
    },
    {
        ...
        "roc_auc_mean": 0.5849,
        "note": "Target baseline before N9.",
    },
]))
