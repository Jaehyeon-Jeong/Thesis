# Kaggle Stage 4 N6 News-Context Baseline Controls One Cell
...
news vector: train-only TF-IDF/SVD, 32 dims per window
context vector: 96 SVD features + 6 log-count features = 102 dims
model: CNN + news concat
seeds: 42, 43, 44, 45, 46
```

The Stage 2 visual-only `I60/R20/ohlc_ma_vb` five-seed result remains the
reference baseline. N6 checks whether adding headline-only news context through
the simplest concat control improves or destabilizes the selected visual
baseline before moving to FiLM-news experiments.

Disk-space note:
- Default settings do not create per-step backup zips.
- At the end, one compact bundle is written to
...
SVD_DIM = 32
CONTEXT_FEATURE_SET_NAME = "news_tfidf_svd_w7_20_60"
PREBUILT_CONTEXT_NAME = "stage4_news_context_i60_ohlc_ma_vb_r20_tfidf_svd_w7_20_60"
CONTEXT_METHOD = "concat"
...
# Smoke check only. For the real N6 run, keep False.
SMOKE_TEST = False

# Stage 2-style comparison settings.
BATCH_SIZE = 128
NUM_WORKERS = 2
...
OUTPUT_PREFIX = "stage4_news_context_n6_concat_five_seed"
BUNDLE_PATH = Path("/kaggle/working/stage4_news_context_n6_result_bundle.zip")
...
def context_seed_dir(run_seed: int) -> Path:
    return PROJECT_ROOT / "outputs/stage4/context" / PREBUILT_CONTEXT_NAME / f"seed_{run_seed}"
...
def patch_prebuilt_news_config(feature_order: list[str]):
    """Patch Stage 4 config so model runners read N5 prebuilt news context."""
    ...
    cfg["context"]["prebuilt_context_name"] = PREBUILT_CONTEXT_NAME
    ...
    config_path.write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")
    print("Prebuilt news config patched:", config_path, "context_dim=", len(feature_order), flush=True)


def ensure_news_vector_artifacts():
    """Build N3/N4 news vectors if they are not present in this Kaggle working tree."""

    news_vector_path = (
        PROJECT_ROOT
        / f"outputs/stage4/news/stage4_news_tfidf_svd_i{IMAGE_WINDOW}_r{RETURN_HORIZON}"
        / "news_tfidf_svd_features.parquet"
    )
    if news_vector_path.exists():
        print("News TF-IDF/SVD vectors already exist:", news_vector_path, flush=True)
        return
...
def build_news_context_for_seed(run_seed: int):
    """Build N5 prebuilt context artifact for one seed."""

    scaler = context_seed_dir(run_seed) / "context_scaler.json"
    features = context_seed_dir(run_seed) / "context_features.csv"
    if scaler.exists() and features.exists():
        print(f"News context already exists for seed {run_seed}: {context_seed_dir(run_seed)}", flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_news_context_features.py",
        ...
        "--window-days", *map(str, NEWS_WINDOWS),
        "--feature-set-name", "tfidf_svd_w7_20_60",
        "--write-report-copy",
    ])


def load_feature_order() -> list[str]:
    scaler_path = context_seed_dir(RUN_SEEDS[0]) / "context_scaler.json"
    payload = json.loads(scaler_path.read_text(encoding="utf-8"))
    feature_order = [str(feature) for feature in payload["feature_order"]]
    if len(feature_order) != 102:
        print(f"[warning] expected 102 news context features, got {len(feature_order)}", flush=True)
...
def summarize_seed_results(seed_rows: list[dict]) -> pd.DataFrame:
    ...
    rows = []
    for method, frame in df.groupby("context_method", dropna=False):
...
def write_result_bundle(seed_csv: Path, mean_std_csv: Path, run_summary_json: Path):
    ...
    for pattern in [
        "stage4_news*_audit*.json",
        "stage4_news*_summary*.csv",
        "stage4_news*_manifest*.json",
        "stage4_news_context*_context_feature_*.json",
        "stage4_news_context*_context_feature_*.csv",
    ]:
        include_paths.extend(sorted(tables.glob(pattern)))

    reports_gradcam = PROJECT_ROOT / "reports/figures/gradcam"
    if reports_gradcam.exists():
        include_paths.extend(sorted(reports_gradcam.glob(f"{experiment_name()}_seed_*")))
    ...
    with zipfile.ZipFile(BUNDLE_PATH, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        manifest = {
            ...
            "run_seeds": RUN_SEEDS,
            "context_method": CONTEXT_METHOD,
            "context_name": PREBUILT_CONTEXT_NAME,
...
# ============================================================
# 2. Ensure N3/N4/N5 news context artifacts exist
# ============================================================
ensure_news_vector_artifacts()
for run_seed in RUN_SEEDS:
    build_news_context_for_seed(run_seed)

feature_order = load_feature_order()
patch_prebuilt_news_config(feature_order)


# ============================================================
# 3. Train/evaluate/check CNN + news concat over five seeds
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
for run_seed in RUN_SEEDS:
    ...
    try:
        ...
        if RUN_GRADCAM:
            ...
            print("[check] skipped full output check because RUN_GRADCAM=False", flush=True)
...
display(Markdown("# Stage 4 N6 News Context Seed-Level Results"))
display(seed_df.sort_values(["context_method", "run_seed"]))

display(Markdown("# Stage 4 N6 News Context Mean/Std Summary"))
...
display(Markdown("# Reference Baseline"))
display(pd.DataFrame([
    {
        "reference": "Stage2 selected visual baseline",
        "image_window": 60,
        "return_horizon": 20,
        "image_spec": "ohlc_ma_vb",
        "note": "Compare N6 news-concat against the existing Stage2 five-seed selected baseline.",
    }
]))
