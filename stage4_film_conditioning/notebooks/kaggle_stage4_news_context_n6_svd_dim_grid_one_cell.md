# Kaggle Stage 4 N6.1 News SVD-Dim Grid One Cell
...
This runner executes the N6.1 news-context dimension diagnostic:
...
Purpose:
- N6 with SVD dim `32` used `102` context features and showed useful signal in
  some seeds but collapsed in others.
- N6.1 tests whether smaller news vectors reduce seed collapse before moving to
  N7 bounded FiLM.
...
# Smoke check only. For the real N6.1 run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = "stage4_news_context_n6_svd_dim_grid_concat_five_seed"
BUNDLE_PATH = Path("/kaggle/working/stage4_news_context_n6_svd_dim_grid_result_bundle.zip")
...
def experiment_suffix(svd_dim: int) -> str:
    return f"news_tfidf_svd{int(svd_dim)}_w7_20_60"
...
def patch_prebuilt_news_config(svd_dim: int, feature_order: list[str]):
    ...
    cfg["context"]["feature_set_name"] = experiment_suffix(svd_dim)
...
def write_result_bundle(seed_csv: Path, mean_std_csv: Path, run_summary_json: Path):
    ...
    include_paths.extend(sorted(tables.glob("stage4_news_context_n6_svd_dim_grid*")))
...
# ============================================================
# 3. Train/evaluate CNN + news concat over SVD dims and five seeds
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
display(Markdown("# Stage 4 N6.1 News SVD-Dim Grid Seed-Level Results"))
display(seed_df.sort_values(["svd_dim", "run_seed"]))

display(Markdown("# Stage 4 N6.1 News SVD-Dim Grid Mean/Std Summary"))
