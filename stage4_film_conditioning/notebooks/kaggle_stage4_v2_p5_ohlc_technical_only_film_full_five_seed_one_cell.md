# Kaggle Stage 4 v2 Priority 5 OHLC Technical-Only FiLM-Full Five-Seed One Cell
...
- Fear & Greed Kaggle dataset (still attached for current builder compatibility; not used as primary context in this run):
  `ashishpatel8736/historical-and-fear-greed-index-datasets`

This runner executes Stage 4 v2 priority 5:

```text
I60 / R20 / ohlc / context_window=60
technical-only context x film_full x 5 seeds = 5 runs
...
RETURN_HORIZON = 20
CONTEXT_WINDOW = 60
CONTEXT_FEATURE_SET_NAME = "technical_only"
PRIMARY_CONTEXT_FEATURES = ["bb_percent_b_60", "bb_bandwidth_60", "mfi_60", "rv_60"]
...
# Smoke check only. For the real 4-V4 run, keep False.
SMOKE_TEST = False
...
# ============================================================
# 3. Train/evaluate/check technical-only FiLM-full over five seeds
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
seed_csv = tables_root / "stage4_v2_p5_ohlc_technical_only_film_full_five_seed_seed_results.csv"
mean_std_csv = tables_root / "stage4_v2_p5_ohlc_technical_only_film_full_five_seed_mean_std_results.csv"
run_summary_json = tables_root / "stage4_v2_p5_ohlc_technical_only_film_full_five_seed_run_summary.json"
...
backup_outputs("stage4_v2_p5_ohlc_technical_only_film_full_five_seed", "after_summary", context_method=None, run_seed=None)

display(Markdown("# Stage 4 v2 Priority 5 OHLC Technical-Only FiLM-Full Five-Seed Seed-Level Results"))
display(seed_df.sort_values(["context_method", "run_seed"]))

display(Markdown("# Stage 4 v2 Priority 5 OHLC Technical-Only FiLM-Full Five-Seed Mean/Std Summary"))
if not mean_std_df.empty and "accuracy_mean" in mean_std_df.columns:
