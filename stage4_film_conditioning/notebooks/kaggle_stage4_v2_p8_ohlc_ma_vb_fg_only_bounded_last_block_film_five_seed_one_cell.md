# Kaggle Stage 4 v2 Priority 8 OHLC_MA_VB F&G-Only Bounded Last-Block FiLM Five-Seed One Cell
...
This runner executes Stage 4 v2 priority 8:

```text
I60 / R20 / ohlc_ma_vb / context_window=60
F&G-only context x film_full_bounded_last_block x 5 seeds = 5 runs
...
CONTEXT_METHODS = ["film_full_bounded_last_block"]
...
# Smoke check only. For the real 4-V7 run, keep False.
SMOKE_TEST = False
...
def read_result_row(context_method: str, run_seed: int, status: str, error: str = "") -> dict:
    ...
    row = {
        ...
        "stage4_v2_p7_ohlc_ma_vb_fg_only_film_full_accuracy_mean": 0.5524,
        "stage4_v2_p7_ohlc_ma_vb_fg_only_film_full_roc_auc_mean": 0.5465,
        "bounded_film_scale": BOUNDED_FILM_SCALE,
    }
...
# ============================================================
# 3. Train/evaluate/check F&G-only bounded last-block FiLM over five seeds
# ============================================================
smoke_train_args = []
smoke_data_args = []
if SMOKE_TEST:
...
if not seed_df.empty and "accuracy" in seed_df.columns:
    seed_df["accuracy_minus_stage2_ohlc_ma_vb_mean"] = (
        pd.to_numeric(seed_df["accuracy"], errors="coerce") - 0.579320
    )
    seed_df["roc_auc_minus_stage2_ohlc_ma_vb_mean"] = (
        pd.to_numeric(seed_df["roc_auc"], errors="coerce") - 0.584862
    )
    seed_df["accuracy_minus_p7_film_full_mean"] = (
        pd.to_numeric(seed_df["accuracy"], errors="coerce") - 0.5524
    )
    seed_df["roc_auc_minus_p7_film_full_mean"] = (
        pd.to_numeric(seed_df["roc_auc"], errors="coerce") - 0.5465
    )
if not mean_std_df.empty and "accuracy_mean" in mean_std_df.columns:
    mean_std_df["accuracy_mean_minus_stage2_ohlc_ma_vb_mean"] = (
        pd.to_numeric(mean_std_df["accuracy_mean"], errors="coerce") - 0.579320
    )
    mean_std_df["roc_auc_mean_minus_stage2_ohlc_ma_vb_mean"] = (
        pd.to_numeric(mean_std_df["roc_auc_mean"], errors="coerce") - 0.584862
    )
    mean_std_df["accuracy_mean_minus_p7_film_full_mean"] = (
        pd.to_numeric(mean_std_df["accuracy_mean"], errors="coerce") - 0.5524
    )
    mean_std_df["roc_auc_mean_minus_p7_film_full_mean"] = (
        pd.to_numeric(mean_std_df["roc_auc_mean"], errors="coerce") - 0.5465
    )

seed_csv = tables_root / "stage4_v2_p8_ohlc_ma_vb_fg_only_bounded_last_block_film_five_seed_seed_results.csv"
mean_std_csv = tables_root / "stage4_v2_p8_ohlc_ma_vb_fg_only_bounded_last_block_film_five_seed_mean_std_results.csv"
run_summary_json = tables_root / "stage4_v2_p8_ohlc_ma_vb_fg_only_bounded_last_block_film_five_seed_run_summary.json"
...
backup_outputs("stage4_v2_p8_ohlc_ma_vb_fg_only_bounded_last_block_film_five_seed", "after_summary", context_method=None, run_seed=None)

display(Markdown("# Stage 4 v2 Priority 8 OHLC_MA_VB F&G-Only Bounded Last-Block FiLM Five-Seed Seed-Level Results"))
display(seed_df.sort_values(["context_method", "run_seed"]))

display(Markdown("# Stage 4 v2 Priority 8 OHLC_MA_VB F&G-Only Bounded Last-Block FiLM Five-Seed Mean/Std Summary"))
if not mean_std_df.empty and "accuracy_mean" in mean_std_df.columns:
