# Kaggle Stage 5 5-8A Embedding FiLM Ablation One Cell
...
The cell rebuilds Stage 4 prebuilt context artifacts from the Stage 5 compact
feature table, then trains/evaluates the bounded FiLM correction model.
...
FEATURE_SET_NAME = f"stage5_embedding_{EMBEDDING_AGGREGATION}_dim{EMBEDDING_DIMENSION}"
EXPERIMENT_SUFFIX = f"{FEATURE_SET_NAME}_pretrained_frozen_{SCALE_LABEL}"
CONTEXT_NAME = (
    f"stage5_embedding_context_i{IMAGE_WINDOW}_{IMAGE_SPEC}_"
    f"r{RETURN_HORIZON}_embedding_{EMBEDDING_AGGREGATION}_dim{EMBEDDING_DIMENSION}"
)
...
# Smoke check only. For the real 5-8A run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = (
    f"stage5_8a_embedding_{EMBEDDING_AGGREGATION}_dim{EMBEDDING_DIMENSION}_"
...
for run_seed in RUN_SEEDS:
    print("\n" + "=" * 90, flush=True)
    print(f"5-8A {stage4_experiment_name()} seed={run_seed}", flush=True)
    print("=" * 90, flush=True)
...
display(Markdown("# Stage 5 5-8A Seed Results"))
display(seed_df.sort_values(["run_seed"]))

display(Markdown("# Stage 5 5-8A Mean/Std Summary"))
display(mean_std_df)
