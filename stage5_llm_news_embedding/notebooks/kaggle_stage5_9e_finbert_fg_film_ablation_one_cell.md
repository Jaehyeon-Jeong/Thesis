# Kaggle Stage 5 5-9E FinBERT + F&G FiLM Ablation One Cell
...
This runner executes `5-9E`:

```text
image spec: ohlc_ma_vb
visual baseline: Stage2 I60/R20/ohlc_ma_vb checkpoints
context: FinBERT headline sentiment 7/20/60-day window features + F&G-only regime features
feature source: FinBERT positive/negative/neutral probabilities, sentiment, counts, news-FG proxy, and raw F&G value/mean/delta/std
...
The cell first builds Stage 4 F&G context features if they are missing, then
rebuilds Stage 4 prebuilt context artifacts by appending F&G features to the
Stage 5 FinBERT table. It then trains/evaluates the bounded FiLM correction
model.
...
STAGE5_INPUT_CANDIDATES = [
    Path("/kaggle/input/datasets/moskow/stage5/stage5_llm_news_embedding_5_9e_finbert_fg_context_bundle"),
    Path("/kaggle/input/datasets/moskow/stage5/stage5_llm_news_embedding_5_9e_finbert_fg_context_bundle.zip"),
    Path("/kaggle/input/stage5-llm-news-embedding-5-9e-finbert-fg-context-bundle/stage5_llm_news_embedding"),
    Path("/kaggle/input/stage5-llm-news-embedding-5-9e-finbert-fg-context-bundle/stage5_llm_news_embedding_5_9e_finbert_fg_context_bundle.zip"),
...
FG_FEATURES = ["fg_value", "fg_mean_60", "fg_delta_60", "fg_std_60"]
...
# Smoke check only. For the real 5-9E run, keep False.
SMOKE_TEST = False
...
OUTPUT_PREFIX = (
    f"stage5_9e_finbert_fg_sentiment_"
...
def fg_context_name() -> str:
    return (
        f"stage4_context_i{IMAGE_WINDOW}_{IMAGE_SPEC}_"
        f"r{RETURN_HORIZON}_c{CONTEXT_WINDOW}"
    )


def fg_context_features_path() -> Path:
    return (
        PROJECT_ROOT
...
def patch_kaggle_config_for_fg_context_build(stage2_checkpoint_bundle_root: Path):
    ...
    print("Base config patched for F&G context build:", config_path, flush=True)


def build_fg_context_features_if_missing():
    fg_path = fg_context_features_path()
    if fg_path.exists():
        print("F&G context features already exist:", fg_path, flush=True)
        return
    run([
        sys.executable, "-u",
        "scripts/build_stage4_context_features.py",
...
def build_prebuilt_stage4_context():
    run([
        ...
        "--feature-source", "finbert_fg",
        "--feature-prefix", "finbert_",
        "--fg-feature-table", str(fg_context_features_path()),
        "--fg-features", *FG_FEATURES,
        "--aggregation", "mean",
        "--dimension", "0",
...
def read_result_row(run_seed: int, status: str, error: str = "") -> dict:
    ...
    row = {
        ...
        "feature_source": "finbert_fg",
        "fg_features": ",".join(FG_FEATURES),
...
patch_kaggle_config_for_fg_context_build(stage2_checkpoint_bundle_root)
build_fg_context_features_if_missing()
...
for run_seed in RUN_SEEDS:
    print("\n" + "=" * 90, flush=True)
    print(f"5-9E {stage4_experiment_name()} seed={run_seed}", flush=True)
    print("=" * 90, flush=True)
...
display(Markdown("# Stage 5 5-9E Seed Results"))
display(seed_df.sort_values(["run_seed"]))

display(Markdown("# Stage 5 5-9E Mean/Std Summary"))
display(mean_std_df)
