...
Interpretability is based on three views. First, Grad-CAM shows the chart regions that support a prediction. Second, gamma and beta summaries show how strongly context modulates visual channels. Third, conditional bucket analysis shows where the context model helps or harms: uncertain baseline samples, sentiment regimes, news-intensity regimes, and leverage regimes.
...
The correction/regression evidence supports this interpretation. The FinBERT+F\&G model improves more in uncertain visual samples and in active sentiment/news regimes: the strongest positive buckets are near-threshold chart-only predictions, greed regimes, and high-news-count windows. The derivatives model provides a related same-image case: selected correction examples and transition summaries show that funding plus CFTC open-interest context tends to lower some weak bullish predictions. In both cases, the model behaves as a small correction layer rather than a new dominant predictor.

The predefined N16 bucket analysis makes this derivatives result more precise. It should be read as a conditional analysis of the controlled \code{ohlc\_vb} setting, not as a replacement for the strongest \code{ohlc\_ma\_vb} visual model. The full-test same-image improvement is small, but the correction is concentrated in samples where the visual model is uncertain and the funding context is high. Table 9 reports the main predefined buckets. These buckets were defined before looking at their accuracy deltas, which reduces the risk of selecting only favorable regimes after the fact.
...
The largest conditional gain appears in the intersection of visual uncertainty and high funding. In that bucket, the context model reduces the average upward probability and converts more false-Up predictions into correct Down predictions than it creates new errors. This does not make the derivatives model the best overall predictor, because it still uses a weaker \code{ohlc\_vb} visual baseline. It does, however, support the narrower interpretation that leverage context can be useful when the chart-only decision is ambiguous.

These conditional counts should also be read cautiously. Across all derivative-model decision changes in Table 9, the sign-test evidence is weak: the all-test transition count gives $p\approx0.195$. The uncertain-chart and high-funding bucket is more suggestive, with 24 corrections and 12 regressions ($p\approx0.065$ two-sided), but it is still a conditional diagnostic rather than a final significance claim. In addition, 20-day forward-return labels are sampled daily and therefore overlap, so the effective number of independent observations is smaller than the raw row count.

\subsection{Interpretability of FiLM modulation}

The interpretability claim in this thesis is diagnostic rather than causal. Grad-CAM shows which chart regions are salient for the CNN, correction/regression tables show when context changes a hard decision, and FiLM gamma/beta summaries show the magnitude and direction of channel-wise modulation. These artifacts make the context path inspectable, but they do not prove that a particular news item caused the market move.

Grad-CAM is most useful when it is paired with a decision transition or with a visible probability shift. If the hard prediction is unchanged, the heatmap is only a saliency sanity check. Therefore, the main evidence in Section 6 is the correction/regression and conditional-bucket analysis. Figure 7 is kept as a compact qualitative example, while the near-identity gamma/beta values support the interpretation that bounded FiLM acts mainly as conservative probability calibration.

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{gradcam_finbert_fg_corrections_clean.png}
\caption{Example Grad-CAM comparison for selected samples}
\end{figure}
...
For this reason, the final protocol freezes the trained CNN and classifier and trains only the context encoder and FiLM heads. The goal is not to maximize capacity, but to isolate the marginal role of context. If the output probability changes under this protocol, the change is attributable to the bounded context-modulation path rather than to a newly learned visual filter. This makes the interpretation cleaner, especially when the measured effects are small.
