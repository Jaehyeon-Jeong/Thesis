...
\toprule
\textbf{Bucket} & \textbf{N} & \textbf{$\Delta$ Acc.} & \textbf{Corr.} & \textbf{Regr.} & \textbf{Net} & \textbf{$\Delta P(up)$}\\
\midrule
...
2021-05-19 & Corr. & 0.53$\rightarrow$0.49 & F\&G=23; $\Delta_{60}$=-52; neg=.25$>$.23 & Fear transition; false-Up chart decision changes into Down.\\
...
The selected cases show that the final context branch mainly acts as a conservative downward calibration layer. Across the matched test decisions, the average change in upward probability is negative ($\Delta P(up)=-0.0162$). The effect is therefore not a symmetric bullish/bearish controller. It is a context-conditioned downward recalibration whose magnitude varies by regime: the average shift is larger in greed and extreme-greed F\&G buckets than in fear buckets. Figure 7 shows this diagnostic pattern over all matched test decisions. The model mostly lowers $P(up)$, but the magnitude is not a constant offset; it becomes stronger in higher F\&G regimes. In the correction examples, this behavior is useful because the chart-only model is weakly bullish while the context regime is deteriorating, fearful, or negative-news leaning. In the regression examples, the same mechanism becomes harmful because the true outcome is Up and the context still pushes the model below the 0.5 threshold. This is why correction/regression analysis is more informative than only reporting the small five-seed accuracy delta.

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{delta_pup_vs_fg_diagnostic.png}
\caption{Change in $P(up)$ by Fear \& Greed regime}
\end{figure}

The gamma/beta summaries support the same interpretation. Table 11 summarizes four selected correction cases and four selected regression cases across five seeds, giving 20 panel observations per group. The average modulation is very close to the identity transformation. Regression panels have slightly larger norms than correction panels, but both groups remain small in absolute terms. Therefore, bounded FiLM does not rewrite the visual representation. It makes small channel-wise shifts that can move borderline probabilities across the decision threshold.

\begin{table}[H]
\caption{Gamma/beta modulation summary for selected Grad-CAM panels ($n=20$ per group)}
\begin{center}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lrrrr}
...
Figure 8 gives a qualitative view of three correction cases. The chart image is identical between the chart-only and context-FiLM rows, but the predicted probability and Grad-CAM saliency pattern change after context conditioning. Grad-CAM is computed for the predicted class in each row, and each heatmap is normalized per image; therefore, visible saliency differences should not be interpreted as proportional to the small $\gamma/\beta$ magnitudes. The figure should not be read as a causal explanation of why Bitcoin moved down. It only shows that, in these cases, the context-conditioned model used a different visual evidence pattern while applying a bounded gamma/beta modulation.
...
Macro-stress controls provide a negative robustness check. The best OFR FSI row reaches accuracy 0.579875, ROC-AUC 0.584859, and net correction +4, while the best public RORO proxy row reaches accuracy 0.579320, ROC-AUC 0.584748, and net correction 0. These results are stable but weaker than the sentiment and derivatives cases, so they are reported as controls rather than final model candidates.
...
Repository: \url{https://github.com/Jaehyeon-Jeong/Thesis}.
...
Delta-$P(up)$ versus F\&G diagnostic & Checks whether the final FiLM branch is a constant shift or context-dependent downward calibration\\
\bottomrule
\end{longtable}
\end{center}
